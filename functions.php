...
if ( ! function_exists( 'ares_setup' ) ) :
	...
	function ares_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'ares' ),
		) );
...
function ares_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'ares_content_width', 640 );
}
...
function ares_scripts() {
	...
	wp_enqueue_script( 'ares-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'ares-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
